...
# fix for all selftest NA denominator


# province as character
# date for last hiv test
# Med year
...
# Calculating the difference of last hiv test from current year
namphia2017 <- namphia2017 %>%
  mutate(
    # Create a reference date using the year variable
    reference_date = make_date(year, 1, 1),
    
    # Calculate the difference in years
    diff_years = interval(last_hivtest_date, reference_date) / years(1)
  )

#only non-negative values
namphia2017 <- namphia2017 %>%
  mutate(
    diff_years = if_else(diff_years < 0, NA_real_, diff_years)
  )
...
namphia2017 <- namphia2017 %>%
  mutate(
    last_hivtest = case_when(
      diff_years < 1 ~ 1,                                      # Tested <12 months ago
      diff_years >= 1 & diff_years <= 2 ~ 2,                   # Tested 1-2 years ago
      diff_years > 2 ~ 3,                                      # Tested more than 2 years ago
      last_hivtest_year == -9 | last_hivtest_month == -9 ~ 98, # Refused
      last_hivtest_year == -8 | last_hivtest_month == -8 ~ 88, # Don't know
      TRUE ~ NA_integer_                                       # Missing 
    )
  )
...
# Saving
#saveRDS(combined_phia, file = "combined_phia_dataframe.rds")
